...
static void returnStatement() {
    ...
    if (match(TOKEN_SEMICOLON)) {
        ...
        if (current->type == TYPE_INITALIZER) {
            errorAtCurrent("Can't return a value from an initalizer.");
        }
        expression();
...
static void parsePrecedence(Precedence precedence) {
    ...
    while (precedence <= getRule(parser.current.type)->precedence) {
        ...
        ASSERT(infixRule, "%d - %.*s", parser.previous.line, parser.previous.length, parser.previous.start)
        infixRule(canAssign);
    }
...
static int resolveLocal(Compiler* compiler, Token* name) {
    for (int i = compiler->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            if (local->depth == -1) {
                errorAtCurrent("Can't read local variable in its own initalizer.");
            }
            return i;
        }
    }
...
static void declareVariable() {
    ...
    for (int i = current->localCount - 1; i >= 0; i--) {
        ...
        if (identifiersEqual(name, &local->name)) {
            errorAtCurrent("Variables in the same scope cannot share names.");
        }
    }
