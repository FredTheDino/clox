    ...
    if (IS_OBJ(callee)) {
        switch (OBJ_TYPE(callee)) {
            case OBJ_BOUND_METHOD: {
                ...
                vm.stackTop[-argCount - 1] = bound->reciver;
...
static ObjUpvalue* captureUpvalue(Value* local) {
    // You can write this smarter. I know you can. ;)
...
#define READ_CONSTANT_LONG() (frame->closure->function->chunk.constants.values[READ_BYTE() \
                              | READ_BYTE() << 0x08 \
                              | READ_BYTE() << 0x10 \
                              ])
        ...
        switch (instruction = READ_BYTE()) {
            case OP_ADD: {
                if (IS_STRING(peek(0)) && IS_STRING(peek(1))) {
                    ...
                    runtimeError("Operands to `+` must be two number or two strings");
                    return INTERPRET_RUNTIME_ERROR;
                }
                break;
            }
